      ...
      {loading ? (
        ...
        <>
              ...
              return (
                <div key={r.id} className="card-apple p-0 overflow-hidden hover-lift group flex flex-col border-white/60 dark:border-white/5">
                    ...
                    {imageUrl ? (
                      <img
                        ...
                        onError={(e) => {
                          ...
                          e.target.parentElement.innerHTML = '<div class="w-full h-full flex items-center justify-center"><svg class="w-12 h-12 text-slate-300 dark:text-dark-border" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg></div>';
                        }}
                      />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center">
                        <Image className="w-12 h-12 text-slate-300 dark:text-dark-border" />
                      </div>
                    )}
                  ...
                  <div className="p-3 md:p-4 flex-1 flex flex-col">
                    ...
                    <div className="flex items-center gap-2 mt-1 text-[9px] text-slate-400 dark:text-dark-mute">
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-100 dark:border-dark-border/40 flex items-center justify-between">
                      <button onClick={() => handleDelete(r.id)} className="text-slate-300 hover:text-accent-red transition-colors p-1 cursor-pointer" title="Delete">
          ...
          {totalPages > 1 && (
            <div className="mt-8 flex items-center justify-between">
              <span className="text-[10px] text-slate-400 dark:text-dark-mute font-medium">{totalReceipts} total receipts</span>
        ...
        <div className="card-apple p-16 text-center border-white/60 dark:border-white/5">
          <div className="w-20 h-20 rounded-full bg-slate-50 dark:bg-white/[0.02] flex items-center justify-center mx-auto mb-6 border border-dashed border-slate-200 dark:border-white/10">
            <Camera className="w-10 h-10 text-slate-300 dark:text-dark-border" />
          </div>
