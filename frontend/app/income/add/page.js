  ...
  useEffect(() => {
    if (user) {
      api.get('/api/accounts').then(res => {
        setAccounts(res.data?.filter(a => a.is_active !== false) || []);
      }).catch(() => {});
    }
  }, [user]);
  ...
  if (!user) {
    return (
      <div className="max-w-2xl mx-auto text-center py-20">
        <p className="text-sm text-slate-500 dark:text-dark-mute">Sign in to record income.</p>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10 animate-fade-in-up">
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-2">
          <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-500 text-[10px] font-bold uppercase tracking-widest">Capital Inflow</span>
        </div>
        <h1 className="text-2xl md:text-3xl text-slate-900 dark:text-white font-extrabold tracking-tight mb-1">Record Income</h1>
        <p className="text-sm md:text-base text-slate-500 dark:text-dark-mute">Log a new income entry to your financial ledger.</p>
      </div>
